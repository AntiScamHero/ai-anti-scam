...
(function(){
  ...
  function installResultGuardStyle(){
    // 樣式已移到 style.css；這裡只保留函式避免舊呼叫失效。
  }
...
const MAX_FAMILY_RECORDS = 50;
    ...
    */
    ...
    async function getApiHeadersForFamily(familyID, options = {}) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (myPasteButton) {
    ...
    async function copyFamilyInviteLink() {
      ...
      try {
        if (navigator.share) {
          const shareData = inviteLink
            ? { title: "AI防詐盾牌家庭守護邀請", text, url: inviteLink }
            : { title: "AI防詐盾牌家庭守護邀請", text };
          await navigator.share(shareData);
          setFamilySetupStatus("已開啟分享邀請。家人點連結會自動加入；如果沒有自動加入，再輸入代碼即可。", "success");
          return;
        }
      } catch (e) {}
      ...
      try {
        ...
        setFamilySetupStatus("家庭邀請文字已複製，可以貼到 LINE 給家人。", "success");
      } catch (e) {
...
function getLineBotName() {
  return getLineConfigValue("LINE_BOT_NAME", "AI防詐盾牌") || "AI防詐盾牌";
}

function getLineBotBasicId() {
  const raw = getLineConfigValue("LINE_BOT_BASIC_ID", "").replace(/^＠/, "@").trim();

  // 避免把「@你的LINE官方帳號BasicID」這種範例字串拿去產生壞掉的 LINE 連結。
  if (!raw || /你的|請填|basicid|example|demo/i.test(raw)) return "";
...
function buildLineOaMessageUrl(bindText) {
  ...
  if (!botId || !String(bindText || "").trim()) return "";
...
async function fetchLineApiWithFreshToken(apiPath, familyID, body = {}, timeoutMs = 15000) {
  const url = buildLineApiUrl(apiPath);
  ...
  // Render 重新部署後，如果沒有固定 AI_SHIELD_TOKEN_SECRET，手機端舊 accessToken 會失效。
  // 這裡遇到 401/403 會自動重新拿 token 再試一次，避免使用者只能看到籠統的 LINE 失敗。
  if ((response.status === 401 || response.status === 403) && typeof ensureDemoAccessToken === "function") {
    try {
      await ensureDemoAccessToken({ forceRefresh: true, familyID });
      response = await fetchWithTimeout(url, {
        method: "POST",
        headers: await getApiHeadersForFamily(familyID, { forceRefresh: true }),
        body: payload
      }, timeoutMs);
      data = await response.json().catch(() => ({}));
    } catch (refreshError) {
      console.warn("LINE API 重新取得 accessToken 後仍失敗：", refreshError);
    }
  }
...
function buildLineInviteMessage(invite = {}) {
  ...
  return [
    ...
    safeUrlLine,
    "",
    "若 LINE 沒有自動帶入文字，請複製上面的 LINE 綁定文字傳給 AI 防詐盾牌。"
  ].filter(Boolean).join("\n");
}
...
async function requestLineInvite() {
  ...
  try {
    const { response, data } = await fetchLineApiWithFreshToken(apiPath, familyID, {
      family_id: familyID,
      familyID,
      familyId: familyID,
      source: "mobile_app"
    ...
    if (!response.ok || (data.status && data.status !== "success" && !data.inviteToken && !data.lineInviteUrl && !data.data)) {
      throw new Error(data.message || data.error || `HTTP ${response.status}`);
    }
    ...
    if (!invite.bindText) {
      invite.bindText = buildLineBindText(familyID, invite.inviteToken);
    }

    if (!invite.inviteToken && !/\b[A-Z0-9]{6}\s+[A-Z0-9]{6,16}\b/.test(String(invite.bindText || "").toUpperCase())) {
      throw new Error("後端沒有回傳 LINE 綁定邀請碼，已停止產生無效邀請。");
    }

    if (!invite.lineInviteUrl) {
      invite.lineInviteUrl = buildLineOaMessageUrl(invite.bindText);
    }

    if (!invite.lineInviteUrl) {
      throw new Error("尚未設定 LINE_BOT_BASIC_ID，無法產生 LINE 官方帳號邀請連結。");
    }

    lineInviteCache = invite;
    return invite;
  } catch (error) {
    console.warn("LINE 邀請建立失敗：", error);
    lineInviteCache = null;
    setLineStatusText("LINE 邀請建立失敗，請確認後端 LINE 設定與 Basic ID。", "warn");
    const reason = error?.message || String(error || "未知錯誤");
    alert(`LINE 邀請建立失敗。

實際錯誤：${reason}

請確認：
1. Render 已設定 LINE_BOT_BASIC_ID
2. AI_SHIELD_TOKEN_SECRET 有固定設定，重新部署後手機端 token 才不會失效
3. LINE_CHANNEL_ACCESS_TOKEN / SECRET 正確
4. /api/line/invite 可以正常回傳 inviteToken`);
    return null;
  }
}

async function shareTextToLine(message, shareUrl = "") {
  const text = String(message || "");
  const lineUrl = String(shareUrl || buildLineShareUrl(text));

  // APK / Android WebView：優先交給原生層處理 LINE 分享。
  const bridgeCandidates = [
    window.AndroidShare,
    window.AndroidBridge,
    window.Android,
    window.webkit?.messageHandlers?.AndroidShare
  ].filter(Boolean);

  for (const bridge of bridgeCandidates) {
    try {
      if (typeof bridge.shareToLine === "function") {
        bridge.shareToLine(text);
        return true;
      }
      if (typeof bridge.postMessage === "function") {
        bridge.postMessage(JSON.stringify({ type: "shareToLine", text }));
        return true;
      }
    } catch (error) {
      console.warn("原生 LINE 分享橋接失敗，改用網頁分享：", error);
    }
  }
  ...
  try {
    ...
    // 使用者取消分享也會進來，不需要視為致命錯誤。
    console.warn("系統分享未完成，改用 LINE 分享連結：", error);
  }
  ...
  try {
    ...
    console.warn("LINE 分享連結開啟失敗：", error);
  }
...
async function openLineInvite() {
  ...
  const shareUrl = buildLineShareUrl(message);
  setLineStatusText("已產生 LINE 邀請，請選擇要傳給哪位家人。", "success");

  const shared = await shareTextToLine(message, shareUrl);
  if (!shared) {
    await copyAppText(message, "LINE 邀請文字已複製，請貼給家人。");
  }
}
...
async function loadLineBindStatus(options = {}) {
  ...
  try {
    const { response, data } = await fetchLineApiWithFreshToken(apiPath, familyID, { family_id: familyID, familyID, familyId: familyID, source: "mobile_app" }, 15000);
...
async function unbindLineTarget(item = {}) {
  ...
  try {
    const { response, data } = await fetchLineApiWithFreshToken(apiPath, familyID, { family_id: familyID, familyID, id: bindingId, bindingID: bindingId, bindingId, binding_id: bindingId, line_target_id: bindingId }, 15000);
    if (!response.ok || (data.status && data.status !== "success")) throw new Error(data.message || data.error || `HTTP ${response.status}`);
...
(function() {
  document.addEventListener("DOMContentLoaded", () => {
    setTimeout(() => {
      ...
      // 3. 自動偵測連線狀態
      setInterval(() => {
        const isBound = normalizeFamilyCode(
          (typeof getFamilyID === "function" ? getFamilyID() : "") ||
          localStorage.getItem("aiShieldWelcomeFamilyID") ||
          localStorage.getItem("aiShieldPrimaryFamilyID") ||
      ...
      function getRealFamilyCode() {
        // 第一順位：主系統目前確認中的家庭代碼。
        if (typeof getFamilyID === "function") {
          const mainCode = normalizeFamilyCode(getFamilyID());
          if (mainCode && mainCode.length === 6) return mainCode;
        }

        // 第二順位：畫面上已顯示的家庭代碼。
        const legacyCodeText = document.getElementById("familyCodeText");
        if (legacyCodeText && legacyCodeText.innerText) {
          const shownCode = normalizeFamilyCode(legacyCodeText.innerText);
          if (shownCode && shownCode.length === 6) return shownCode;
        }
        ...
        for (const key of keys) {
          const code = normalizeFamilyCode(localStorage.getItem(key));
          if (code && code.length === 6) return code;
        }

        // 第四順位：輸入框裡正在輸入的代碼。
        if (inputEl && inputEl.value) {
          const inputCode = normalizeFamilyCode(inputEl.value);
          if (inputCode && inputCode.length === 6) {
            localStorage.setItem("family_code", inputCode);
            return inputCode;
          }
        }
      ...
      async function handleLineInvite() {
        ...
        if (!currentCode) {
          alert("請先建立或輸入 6 碼家庭代碼，再邀請家人。");
          return;
        }

        // 改走正式 LINE 綁定流程：由後端產生 inviteToken，再開 LINE 分享。
        if (typeof window.openLineInvite === "function") {
          await window.openLineInvite();
          return;
        }

        // 極端備援：正式流程尚未載入時，至少分享家庭代碼文字。
        const inviteMsg = `這是我在用的防詐盾牌，我的家人連線數字是：【 ${currentCode} 】，請幫我在手機輸入連線，一起守護我們家！`;
        const shareUrl = `https://line.me/R/share?text=${encodeURIComponent(inviteMsg)}`;
        if (typeof shareTextToLine === "function") {
          const ok = await shareTextToLine(inviteMsg, shareUrl);
          if (ok) return;
        }
...
/* =========================================================
   從 index.html 移出的互動腳本：之後功能統一在 app.js 維護
   ========================================================= */


/* --- moved from index.html <script id="inline-script"> --- */
window.addEventListener("DOMContentLoaded", function () {
      document.querySelectorAll(".mascot-video").forEach(function(video){
...
/* --- moved from index.html <script id="inline-script"> --- */
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".mascot-fallback").forEach(el => el.remove());
...
/* --- moved from index.html <script id="replace-5769-video-js"> --- */
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".mascot-fallback, img.mascot").forEach(function(el){ el.remove(); });
  const v = document.querySelector(".mascot-video");
  if (!v) return;
  v.muted = true;
  v.loop = true;
...
/* --- moved from index.html <script id="formal-app-redesign-js"> --- */
document.addEventListener("DOMContentLoaded", function () {
  const questions = [
    {
      q: "收到『包裹配送失敗，請立即補繳運費』的簡訊，下一步怎麼做？",
      options: ["立刻點連結付款", "先到官方 App 或 165 查證"],
      correct: 1,
      feedback: "答對了！補繳運費連結常會導向假網站，請先用官方 App 或客服查證。"
    },
    {
      q: "投資群組說『老師帶單、保證獲利、名額有限』，可以加入嗎？",
      options: ["不要加入，先查證來源", "趕快加入，怕錯過機會"],
      correct: 0,
      feedback: "答對了！保證獲利、老師帶單、限時名額，是假投資常見話術。"
    },
    {
      q: "有人自稱檢警，說案件偵查不公開，不能告訴家人，該怎麼辦？",
      options: ["照指示保密", "掛掉並打 165 或問家人"],
      correct: 1,
      feedback: "答對了！真正的檢警不會用通訊軟體要求匯款，也不會叫你不能告訴家人。"
    }
  ];
  let index = 0;
  const qEl = document.getElementById("guardianQuestionText");
  const fb = document.getElementById("guardianFeedback");
  const next = document.getElementById("guardianNextBtn");
  const optionBox = document.querySelector(".guardian-options");

  function renderQuestion() {
    const item = questions[index % questions.length];
    if (qEl) qEl.textContent = item.q;
    if (fb) fb.textContent = "選一個答案，小安心會告訴你原因。";
    if (!optionBox) return;
    ...
    item.options.forEach(function(text, i){
      ...
      btn.addEventListener("click", function(){
        optionBox.querySelectorAll("button").forEach(function(b){ b.disabled = true; });
        btn.classList.add(i === item.correct ? "is-correct" : "is-wrong");
        if (i !== item.correct) {
          const correctBtn = optionBox.querySelectorAll("button")[item.correct];
          if (correctBtn) correctBtn.classList.add("is-correct");
          if (fb) fb.textContent = "這題要小心：" + item.feedback;
        } else {
          if (fb) fb.textContent = item.feedback;
        }
      });
      optionBox.appendChild(btn);
    });
  }
  if (next) next.addEventListener("click", function(){ index += 1; renderQuestion(); });
  renderQuestion();

  document.querySelectorAll('[data-action="go-scan"], [data-action="go-family"]').forEach(function(btn){
    btn.addEventListener("click", function(){
      const action = btn.dataset.action;
      if (action === "go-family" && typeof go === "function") return go("family");
...
/* --- moved from index.html <script id="smart-clipboard-home-js"> --- */
document.addEventListener("DOMContentLoaded", function () {
  const card = document.getElementById("smartClipboardCard");
  const preview = document.getElementById("smartClipboardPreview");
  const checkBtn = document.getElementById("smartClipboardCheckBtn");
  const dismissBtn = document.getElementById("smartClipboardDismissBtn");
  let clipboardCandidate = "";

  function looksWorthChecking(text) {
    const value = String(text || "").trim();
    if (value.length < 8) return false;
    return /(https?:\/\/|line\.me|bit\.ly|reurl\.cc|tinyurl|驗證碼|信用卡|補繳|運費|包裹|投資|老師|群組|帳戶|凍結|匯款|轉帳|檢察官|警察|健保|中獎|補助|USDT|高收益|保證獲利)/i.test(value) || value.length >= 28;
  }
  ...
  function showClipboardPrompt(text, passive) {
    if (!card) return;
    clipboardCandidate = String(text || "").trim();
    if (preview) {
      preview.textContent = clipboardCandidate
        ? "偵測到：「" + shortPreview(clipboardCandidate) + "」"
        : "若你剛複製簡訊、LINE 或網址，可以一鍵帶入檢查。";
    }
    card.style.display = "grid";
    if (!passive) card.scrollIntoView({ behavior: "smooth", block: "nearest" });
  }

  async function readClipboardIfAllowed(passive) {
    if (!navigator.clipboard || !navigator.clipboard.readText) {
      if (!passive) showClipboardPrompt("", false);
      return;
    }
    try {
      const text = await navigator.clipboard.readText();
      if (looksWorthChecking(text)) showClipboardPrompt(text, passive);
      else if (!passive) showClipboardPrompt("", false);
    } catch (error) {
      if (!passive) showClipboardPrompt("", false);
    }
  }

  async function tryPassiveClipboard() {
    try {
      if (!navigator.permissions || !navigator.permissions.query) return;
      const permission = await navigator.permissions.query({ name: "clipboard-read" });
      if (permission && permission.state === "granted") readClipboardIfAllowed(true);
    } catch (e) {}
  }

  if (checkBtn) {
    checkBtn.addEventListener("click", async function () {
      if (!clipboardCandidate) await readClipboardIfAllowed(false);
  ...
  if (dismissBtn && card) dismissBtn.addEventListener("click", function () { card.style.display = "none"; });

  // 使用者回到 App 時，如果瀏覽器已允許剪貼簿權限，就溫和地提示，不強迫讀取。
  tryPassiveClipboard();
  document.addEventListener("visibilitychange", function () {
    if (!document.hidden) tryPassiveClipboard();
  });
});


/* --- moved from index.html <script id="tip-inline-rotate-js"> --- */
(function(){
  const tips = [
    "驗證碼、OTP、簡訊碼，不要提供給任何人。",
    "ATM 無法解除分期付款，客服要求操作 ATM 請立刻停下來。",
    "假冒銀行、電商、物流客服，常會要求你操作網銀或 ATM。",
    "警察、檢察官不會要求你把錢匯到監管帳戶。",
    "對方說不能告訴家人，通常就是詐騙話術。",
    "保證獲利、穩賺不賠、老師帶單，請提高警覺。",
    "飆股群組、主力內線、VIP 明牌，常是詐騙集團包裝。",
    "要求買 USDT 入金投資，請先確認平台是否真實合法。",
    "出金前要求繳保證金、稅金、解凍費，多半是詐騙。",
    "陌生人要求安裝遠端控制 App，請先拒絕。"
  ];
  let index = 0;
  let timer = null;
  function getTipEl(){
    return document.querySelector('#screen-home .hero-inline-tip #antiScamTipText') || document.getElementById('antiScamTipText');
  }
  ...
  function startAutoTips(){
    ...
    setTip(0);
    timer = setInterval(function(){ setTip(index + 1); }, 6500);
  }
  document.addEventListener('DOMContentLoaded', startAutoTips);
  setTimeout(startAutoTips, 600);
  window.aiShieldStartAutoTips = startAutoTips;
})();


/* --- moved from index.html <script id="ai-shield-family-button-final-fallback"> --- */
(function(){
  function bind(){
    if (window.aiShieldBindFamilyButtons) window.aiShieldBindFamilyButtons();
    var a=document.getElementById('familyShareCareBtn'), b=document.getElementById('familyCopy165Btn');
    if(a){a.disabled=false;a.style.pointerEvents='auto';}
...
/* --- moved from index.html <script id="home-final-navigation-fix"> --- */
document.addEventListener("DOMContentLoaded", function () {
  function safeGo(screen) {
    ...
    document.querySelectorAll(".screen").forEach(function(el) {
      var active = el.id === "screen-" + screen;
  ...
  document.addEventListener("click", function (event) {
    ...
    var nav = event.target.closest(".nav-btn[data-screen]");
    if (nav) {
      event.preventDefault();
      safeGo(nav.dataset.screen || "home");
    }
  }, true);
});


/* --- moved from index.html <script id="home-family-status-sync-js"> --- */
document.addEventListener("DOMContentLoaded", function () {
  function getFamilyCode() {
    var el = document.getElementById("familyCodeText");
    var text = el ? String(el.textContent || "").trim() : "";
    if (text && text !== "尚未綁定" && text !== "--") return text;

    var keys = ["familyID", "familyId", "family_id", "AI_SHIELD_FAMILY_ID", "aiShieldFamilyID", "familyCode"];
  ...
  function updateHomeFamilyStatus() {
    var title = document.getElementById("homeFamilyStatusTitle");
    var desc = document.getElementById("homeFamilyStatusDesc");
    if (!title || !desc) return;

    var code = getFamilyCode();
    if (code) {
      title.textContent = "✅ 已連線";
      desc.textContent = "家人可收到高風險提醒";
      title.classList.add("is-connected");
    } else {
      title.textContent = "尚未連線";
      desc.textContent = "連線後，家人可以幫忙確認可疑訊息。";
      title.classList.remove("is-connected");
    }
  }

  updateHomeFamilyStatus();
  setTimeout(updateHomeFamilyStatus, 350);
  setTimeout(updateHomeFamilyStatus, 1200);
  document.addEventListener("click", function () {
    setTimeout(updateHomeFamilyStatus, 250);
  });
  window.aiShieldUpdateHomeFamilyStatus = updateHomeFamilyStatus;
});


/* --- moved from index.html <script id="ai-shield-safe-nav-js"> --- */
document.addEventListener("DOMContentLoaded", function () {
  var screens = ["home", "scan", "family", "education", "result"];

  function setScreen(screen) {
    screen = String(screen || "home").replace(/^screen-/, "");
    if (screens.indexOf(screen) < 0) screen = "home";

    document.body.dataset.currentScreen = screen;

    document.querySelectorAll(".screen").forEach(function (el) {
      var isActive = el.id === "screen-" + screen;
      el.classList.toggle("active", isActive);
      el.hidden = !isActive;
      el.setAttribute("aria-hidden", isActive ? "false" : "true");
    });

    var navScreen = screen === "result" ? "scan" : screen;
    document.querySelectorAll(".bottom-nav .nav-btn[data-screen]").forEach(function (btn) {
      var isActive = btn.dataset.screen === navScreen;
  ...
  document.addEventListener("click", function (event) {
    var nav = event.target.closest(".bottom-nav .nav-btn[data-screen]");
    if (nav) {
      event.preventDefault();
    ...
    if (value === "go-scan") {
      event.preventDefault();
      setScreen("scan");
    } else if (value === "go-family") {
      event.preventDefault();
      setScreen("family");
    } else if (value === "go-education") {
      event.preventDefault();
      setScreen("education");
    } else if (value === "go-home") {
      event.preventDefault();
      setScreen("home");
    }
  }, true);
  ...
  var active = document.querySelector(".screen.active");
  setScreen(active ? active.id.replace(/^screen-/, "") : "home");
  window.aiShieldSafeSetScreen = setScreen;
});
