...
function getDomainFromReportUrl(value = "") {
...
async function addUserReportedDomainFromPopup(url, record = {}) {
    const domain = getDomainFromReportUrl(url);
    if (!domain) return "";
    ...
    try {
        ...
        const nextItem = {
            ...
            url: String(url || ""),
            title: record.title || "",
            familyID: record.familyID || currentFamilyID || "none",
            reason: record.reported_reason || record.reason || "使用者從 Popup 回報此頁為可疑網站。",
            reportedAt: record.reportedAt || new Date().toISOString(),
            source: "popup_report_button",
            status: "watchlist"
        };

        const filtered = current.filter(item => getDomainFromReportUrl(item?.domain || item?.url || "") !== domain);
...
function getLocalStorageFamilySnapshot() {
    const snapshot = {};
    for (const key of FAMILY_ID_SYNC_KEYS) {
        try {
    ...
    try {
        const primary = localStorage.getItem("aiShieldPrimaryFamilyID");
        if (primary) snapshot.aiShieldPrimaryFamilyID = primary;
    } catch (e) {}

    try {
        const saved = localStorage.getItem("savedFamilyID");
        if (saved) snapshot.savedFamilyID = saved;
    } catch (e) {}

    return snapshot;
}

async function getMergedFamilyStorage() {
    let chromeStorage = {};

    try {
        chromeStorage = await chrome.storage.local.get(["userID", ...FAMILY_ID_SYNC_KEYS]);
    } catch (e) {
        chromeStorage = {};
    }

    const localSnapshot = getLocalStorageFamilySnapshot();

    // welcome 有時先寫入 localStorage；popup 讀取時要一起合併。
    const merged = {
        ...localSnapshot,
        ...chromeStorage
    };

    const localCode = pickBestFamilyIDFromStorage(localSnapshot);
    const chromeCode = pickBestFamilyIDFromStorage(chromeStorage);

    // 若 chrome.storage 沒有，但 localStorage 有，立即補回 chrome.storage.local。
    if (localCode && !chromeCode) {
        try {
            await chrome.storage.local.set(buildSharedFamilyPayload(localCode));
            Object.assign(merged, buildSharedFamilyPayload(localCode));
        } catch (e) {}
    }
...
async function forceSyncFamilyFromAllStorage() {
    const merged = await getMergedFamilyStorage();
    const nextFamilyID = pickBestFamilyIDFromStorage(merged);

    if (nextFamilyID) {
        currentFamilyID = nextFamilyID;
        await persistSharedFamilyID(nextFamilyID);
        updateUIAsBound(nextFamilyID);
        return nextFamilyID;
    }
...
function buildInviteMessage(familyID = currentFamilyID) {
    const code = normalizeFamilyCode(familyID);
    if (!code) return "";
...
function buildLineSchemeUrl(familyID = currentFamilyID) {
    const code = normalizeFamilyCode(familyID);
    if (!code) return "";

    // QR Code 容量有限，這裡只放短訊息，讓掃描後 LINE 可以開啟並帶出代碼。
    return `line://msg/text/${encodeURIComponent(`aishield:${code}`)}`;
}
...
function makeQrMatrixV2L(text) {
    const bytes = Array.from(new TextEncoder().encode(String(text || "")));

    // Version 2-L byte capacity is 32 bytes. Data codewords: 34, ECC: 10.
    if (bytes.length > 32) {
        throw new Error("QR 資料過長，請改用複製邀請碼。");
    }
    ...
    appendBits(dataBits, 0, Math.min(4, 272 - dataBits.length));
    ...
    for (let pad = 0; dataCodewords.length < 34; pad++) {
        dataCodewords.push(pad % 2 === 0 ? 0xEC : 0x11);
    }

    const ecc = rsRemainder(dataCodewords, 10);
    ...
    for (let i = 0; i < 8; i++) reserve.push([size - 1 - i, 8]);
    for (let i = 8; i < 15; i++) reserve.push([8, size - 15 + i]);
    reserve.forEach(([x, y]) => {
        if (matrix[y] && matrix[y][x] === null) matrix[y][x] = false;
    });
...
function renderFamilyQRCode() {
    // 家庭邀請流程已簡化：不再顯示 QR Code，只保留「傳給家人」。
}
...
async function shareInviteToLine() {
    ...
    try {
        ...
        console.warn("邀請訊息複製失敗，仍開啟分享頁：", e);
    }

    const helperUrl = chrome.runtime.getURL(`pages/line_share.html?code=${encodeURIComponent(code)}`);

    try {
        await chrome.tabs.create({ url: helperUrl });
        showToast("已準備邀請訊息，請傳給家人。", "success");
    } catch (e) {
        window.open(helperUrl, "_blank");
        showToast("已準備邀請訊息。", "success");
    }
}
...
function syncFamilyDuplicateCodeUI() {
    const isBound = normalizeFamilyCode(currentFamilyID) !== "";
    const joinRow = document.getElementById("family-join-row");
    const btnCreateFamily = document.getElementById("btn_create_family");
    const settingsNote = document.querySelector(".settings-content > .settings-note:first-child");

    if (joinRow) joinRow.style.display = isBound ? "none" : "";
    if (btnCreateFamily) btnCreateFamily.style.display = isBound ? "none" : "";
    if (settingsNote) settingsNote.style.display = isBound ? "none" : "";
}
...
function setLoading(isLoading) {
    ...
    if (scanBtn && scanBtn.dataset.isEscape !== "true") {
        ...
        scanBtn.textContent = isLoading ? "AI 深度分析中..." : "啟動 AI 掃描";
    }
}
...
function renderScanResult(reportData) {
    ...
    if (score >= getRiskThresholdHigh()) {
        ...
        captureEvidenceSnapshot({
            silent: true,
            source: "auto_high_risk_scan",
            reportData
        }).catch(error => {
            console.warn("高風險證據快照自動保存失敗：", error?.message || error);
        });
...
function setEvidenceStatus(message, type = "info") {
    ...
    if (type === "saved") status.classList.add("saved");
    if (type === "error") status.classList.add("error");
}

function renderEvidenceSnapshot(record) {
    latestEvidenceSnapshot = record || null;

    const wrap = document.getElementById("evidence-preview-wrap");
    const img = document.getElementById("evidence-preview-img");
    const meta = document.getElementById("evidence-meta");

    // Popup 不再放完整證據快照卡片；證據統一到家庭戰情室查看。
    // 若舊版 HTML 還留有預覽元件，仍可相容顯示。
    if (!record || !record.imageData) {
        if (wrap) wrap.style.display = "none";
        if (typeof setEvidenceStatus === "function") {
            setEvidenceStatus("尚未保存快照。若頁面可疑，請先拍下證據再離開。", "info");
        }
        return;
    }

    if (img) img.src = record.imageData;

    if (meta) {
        const score = Number(record.riskScore || 0);
        const level = record.riskLevel || "未標示";
        const url = record.url || "未取得網址";
        meta.textContent = `保存時間：${formatEvidenceTime(record.capturedAt)}｜風險：${score} 分｜等級：${level}｜網址：${url}`;
    }

    if (wrap) wrap.style.display = "block";

    if (typeof setEvidenceStatus === "function") {
        setEvidenceStatus("已保存證據快照。請到家庭戰情室查看與匯出。", "saved");
    }
}

function captureVisibleTabAsImage(windowId) {
    return new Promise((resolve, reject) => {
        if (!chrome?.tabs?.captureVisibleTab) {
            reject(new Error("目前環境不支援頁面快照功能。"));
            return;
        }
        ...
        chrome.tabs.captureVisibleTab(
            ...
            dataUrl => {
                ...
                if (err) {
                    reject(new Error(err.message || "無法擷取目前頁面畫面。"));
                    return;
                }

                if (!dataUrl) {
                    reject(new Error("未取得快照資料。"));
                    return;
                }
...
async function saveEvidenceSnapshotRecord(record) {
    const storage = await chrome.storage.local.get([EVIDENCE_SNAPSHOT_STORAGE_KEY]);
    const records = Array.isArray(storage[EVIDENCE_SNAPSHOT_STORAGE_KEY])
        ? storage[EVIDENCE_SNAPSHOT_STORAGE_KEY]
        : [];

    records.unshift(record);

    // 截圖較佔空間，只保留最近 8 筆，避免 storage.local 太大。
    await chrome.storage.local.set({
        [EVIDENCE_SNAPSHOT_STORAGE_KEY]: records.slice(0, 8)
    });
}

async function loadLatestEvidenceSnapshot() {
    try {
        const storage = await chrome.storage.local.get([EVIDENCE_SNAPSHOT_STORAGE_KEY]);
        const records = Array.isArray(storage[EVIDENCE_SNAPSHOT_STORAGE_KEY])
            ? storage[EVIDENCE_SNAPSHOT_STORAGE_KEY]
        ...
        if (records[0]) {
            renderEvidenceSnapshot(records[0]);
        }
    } catch (e) {
        console.warn("讀取證據快照失敗：", e);
    }
}

async function captureEvidenceSnapshot(options = {}) {
    const silent = Boolean(options.silent);
    const report = options.reportData || latestScanReport || {};

    try {
        const tab = await getActiveTab();

        if (!tab?.id || !tab?.url) {
            throw new Error("無法取得目前分頁，請先切回可疑頁面後再拍下快照。");
        }

        if (isRestrictedBrowserOrExtensionPage(tab.url)) {
            throw new Error("Chrome 內部頁或擴充功能頁無法截圖，請在一般網頁上使用。");
        }

        setEvidenceStatus("正在保存目前畫面快照，請稍候...", "info");

        const imageData = await captureVisibleTabAsImage(tab.windowId);
        ...
        const record = {
            ...
            reason: report.reason || report.ai_reason || "",
            advice: report.advice || "",
            source: options.source || "manual_popup_capture",
            privacyMode: "local_first_full_snapshot"
        };

        await saveEvidenceSnapshotRecord(record);
        renderEvidenceSnapshot(record);

        if (!silent) {
            showToast("已保存證據快照，可匯出給家人、165 或警方。", "success");
        }

        return record;
    } catch (error) {
        setEvidenceStatus(`快照保存失敗：${error.message}`, "error");

        if (!silent) {
            showToast(`快照保存失敗：${error.message}`, "error");
        }
...
async function buildEvidenceExportImage(record, options = {}) {
    ...
    let y = 126;
    y = drawWrappedText(ctx, `保存時間：${formatEvidenceTime(record.capturedAt)}`, 34, y, canvas.width - 68, 24, 1);
...
async function exportEvidenceSnapshot(masked = false) {
    try {
        let record = latestEvidenceSnapshot;

        if (!record?.imageData) {
            record = await captureEvidenceSnapshot({
                silent: true,
                source: masked ? "auto_before_masked_export" : "auto_before_full_export"
            });
        }

        const dataUrl = await buildEvidenceExportImage(record, { masked });
        const prefix = masked ? "AI防詐盾牌_遮罩版快照" : "AI防詐盾牌_報案版快照";
        const filename = `${prefix}_${safeEvidenceFileName(record.url)}_${Date.now()}.jpg`;

        downloadDataUrl(dataUrl, filename);
        showToast(masked ? "已匯出遮罩版快照。" : "已匯出報案版快照。", "success");
    } catch (error) {
...
async function reportCurrentPage() {
    try {
        ...
        const report = latestScanReport || {};
        const normalizedFamilyID = normalizeFamilyCode(currentFamilyID) || "none";
        const reportedAt = new Date().toISOString();

        const localRecord = {
            id: `user_report_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
            url: tab.url,
            pageUrl: tab.url,
            title: tab.title || "",
            reportedAt,
            ...
            riskScore: normalizeRiskScore(report),
            riskLevel: report.riskLevel || "使用者回報",
            ai_reason: report.reason || "",
            reason: report.reason || "使用者從 Popup 主動回報此頁需要家人或 AI 後續判斷。",
            reported_reason: "使用者從 Popup 回報此頁需要 AI 後續判斷",
            status: "pending_review",
            statusText: "待家人確認",
            action_type: "popup_user_report",
            source: "popup_report_button"
        };
        ...
        try {
            ...
            // 同一網址重複回報時，保留最新一筆，避免戰情室洗版。
            const filtered = reports.filter(item => String(item?.url || item?.pageUrl || "") !== String(localRecord.url));
            filtered.unshift(localRecord);

            await chrome.storage.local.set({ aiShieldUserReports: filtered.slice(0, 100) });
            await addUserReportedDomainFromPopup(tab.url, localRecord);
        } catch (storageError) {
            console.warn("本機回報紀錄寫入失敗：", storageError);
        }

        showToast("已加入可疑觀察名單；下次進入此網站會主動提醒。", "success");
        ...
        try {
            ...
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/report_false_positive`, {
                ...
                body: JSON.stringify({
                    ...
                    familyID: auth.familyID || normalizedFamilyID || "none",
                    riskScore: normalizeRiskScore(report),
...
function openDemoConsole() {
    ...
    const consoleUrl = familyID
        ? chrome.runtime.getURL(`pages/demo_console.html?familyID=${encodeURIComponent(familyID)}`)
        : chrome.runtime.getURL("pages/demo_console.html");
...
function bindEvents() {
    ...
    const btnToggleQR = document.getElementById("btn_toggle_qr");
    const btnCaptureEvidence = document.getElementById("btn_capture_evidence");
    const btnExportEvidence = document.getElementById("btn_export_evidence");
    const btnExportMaskedEvidence = document.getElementById("btn_export_masked_evidence");
if (scanBtn) {
    ...
    if (btnCaptureEvidence) {
        btnCaptureEvidence.addEventListener("click", () => {
            captureEvidenceSnapshot({ source: "manual_button_capture" }).catch(() => {});
        });
    }

    if (btnExportEvidence) {
        btnExportEvidence.addEventListener("click", () => exportEvidenceSnapshot(false));
    }

    if (btnExportMaskedEvidence) {
        btnExportMaskedEvidence.addEventListener("click", () => exportEvidenceSnapshot(true));
    }
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const storage = await getMergedFamilyStorage();
    ...
    updateUIAsBound(currentFamilyID);
    await forceSyncFamilyFromAllStorage();
    await loadLatestEvidenceSnapshot();
