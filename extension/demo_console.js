    ...
    async function syncFamilyID(familyID = getFamilyID()){
      const code = normalizeFamilyCode(familyID);
      if (code.length !== 6) {
        showToast("目前沒有有效家庭代碼。請先在 welcome.html 或 Popup 建立 / 綁定家庭群組。");
        setFamilyUI("");
    ...
    async function reloadFamilyIDFromStorage(){
      ...
      if (code) {
        ...
        $("syncScopeText").textContent = "同步範圍：尚未綁定家庭代碼";
        showToast("目前尚未找到家庭代碼。請先從 welcome.html 或 Popup 建立家庭。");
      }
      return code;
    }
    ...
    function checkVoice(){
      const audio = new Audio("ai_shield_bilingual_warning.mp3");
      audio.preload = "auto";
      audio.addEventListener("canplaythrough", () => setStatus("voiceStatus","online","Ready","voiceStatusNote","雙語警示音檔可載入。"), {once:true});
      audio.addEventListener("error", () => setStatus("voiceStatus","fallback","Check File","voiceStatusNote","找不到或無法載入 mp3，請確認與 demo_console.html 同層。"), {once:true});
      audio.load();
    }
