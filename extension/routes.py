...
# 引入我們的小幫手
from extensions import limiter, socketio, firebase_initialized
...
def submit_evidence():
    ...
    try:
        ...
        # 🛑 【免付費修改 2】：直接將完整的 Base64 (含檔頭) 存入資料庫，不進行切割！
...
def scan_url():
    ...
    if screenshot_base64 and firebase_initialized:
        cloud_img_url = ""
        # 🛑 【免付費修改 3】：掃描網址時略過 Storage，直接寫入資料庫 (保留完整檔頭)
        try:
...
@api_bp.route('/api/send_family_broadcast', methods=['POST'])
...
@api_bp.route('/api/set_contact', methods=['POST'])
def set_contact():
    data = request.json
    uid = data.get('uid')
...
@api_bp.route('/api/get_contact', methods=['POST'])
def get_contact():
    fid = request.json.get('familyID')
