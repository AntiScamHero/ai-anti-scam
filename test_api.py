...
def test_security_boundary():
    print("\n" + "="*70)
    print("🔒 模組 11: 越權與安全性測試 (相容 Soft Error 版)")
    print("="*70)
    ...
    # 越權測試案例
    cases = [
        {"name": "空白邀請碼加入", "payload": {"uid": "test", "inviteCode": ""}, "expect_fail": True, "endpoint": "/api/join_family"},
        {"name": "超長邀請碼", "payload": {"uid": "test", "inviteCode": "A"*20}, "expect_fail": True, "endpoint": "/api/join_family"},
        {"name": "特殊符號邀請碼", "payload": {"uid": "test", "inviteCode": "@#$%"}, "expect_fail": True, "endpoint": "/api/join_family"},
        {"name": "不存在的家庭ID獲取警報", "payload": {"familyID": "NOEXIST"}, "expect_fail": True, "endpoint": "/api/get_alerts"},
        {"name": "用家庭B的ID獲取家庭A的警報", "use_wrong": True},
        {"name": "清除他人家庭警報", "use_wrong": True},
        {"name": "建立家庭時傳入非字串UID", "payload": {"uid": 12345}, "expect_fail": True, "endpoint": "/api/create_family"},
        {"name": "加入家庭時傳入不存在UID", "payload": {"uid": "no_such_user", "inviteCode": code1}, "expect_fail": False, "endpoint": "/api/join_family"},
    ]
    ...
    for case in cases:
        try:
            if case.get("use_wrong"):
                ...
                res = requests.post(f"{BASE_URL}{endpoint}", json=case["payload"], timeout=TIMEOUT_SEC)
                try:
                    res_json = res.json()
                    # ✨ 完美兼容：如果是 400 算過關，如果是 200 且帶有 fail/error 標籤也算過關！
                    passed = (res.status_code == 400) or (res.status_code == 200 and res_json.get('status') in ['fail', 'error'])
                except:
                    passed = res.status_code == 400
                result.add(case["name"], passed, f"狀態碼: {res.status_code}, 回應: {res.text[:40]}")
            else:
                # 正常情況應成功
                endpoint = case.get("endpoint")
                res = requests.post(f"{BASE_URL}{endpoint}", json=case["payload"], timeout=TIMEOUT_SEC).json()
                passed = res.get('status') == 'success'
    ...
    for case in cases:
        try:
            if case.get("endpoint"):
                ...
                passed = res.status_code in [200, 400, 413]
