...
# 🏁 模組 14: 零預算奪冠優化驗證 (8 筆)
# ==========================================
def test_zero_budget_optimizations():
    print("\n" + "="*70)
    print("🏁 模組 14: 零預算奪冠優化驗證")
    print("="*70)

    try:
        res = requests.get(f"{BASE_URL}/health", timeout=TIMEOUT_SEC)
        data = safe_json_response(res)
        passed = res.status_code == 200 and data.get("status") == "success"
        result.add("Render /health 預熱端點", passed, f"狀態碼: {res.status_code}, 回應: {str(data)[:120]}")
    except Exception as e:
        result.add("Render /health 預熱端點", False, f"錯誤：{str(e)}")

    homograph_cases = [
        {"name": "Punycode 偽裝網域", "url": "https://xn--googl-fsa.com/login", "min_score": 70},
        {"name": "西里爾同形字 google", "url": "https://gооgle.com/login", "min_score": 70},
        {"name": "Userinfo 官方偽裝", "url": "https://google.com@evil-payment.example/login", "min_score": 80},
        {"name": "官方 LINE 網域不誤攔", "url": "https://line.me/R/", "max_score": 20},
    ]
    ...
    for case in homograph_cases:
        try:
            res = session.post(
                f"{BASE_URL}/scan",
                json={"url": case["url"], "text": "請點擊登入驗證帳號"},
                timeout=TIMEOUT_SEC,
            )
            data = safe_json_response(res)
            score = int(data.get("riskScore", 0) or 0)
            if "min_score" in case:
                passed = score >= case["min_score"]
                detail = f"Score: {score}, 預期 >= {case['min_score']}"
            else:
                passed = score <= case["max_score"]
                detail = f"Score: {score}, 預期 <= {case['max_score']}"
            result.add(case["name"], passed, detail)
        except Exception as e:
            result.add(case["name"], False, f"錯誤：{str(e)}")
        time.sleep(0.3)
    ...
    rag_cases = [
        {
            "name": "RAG 案例輸出欄位",
            "text": "您的包裹配送失敗，請立即補繳運費並輸入信用卡資料。",
        },
        {
            "name": "RAG 投資話術案例欄位",
            "text": "老師帶單保證獲利，每日獲利 5%，請加入 LINE 投資群。",
        },
    ]

    for case in rag_cases:
        try:
            res = session.post(f"{BASE_URL}/scan", json={"url": "", "text": case["text"]}, timeout=TIMEOUT_SEC)
            data = safe_json_response(res)
            # 若部署環境有 rag_cases.json，應回傳 similarCases；若沒有，至少不得讓 /scan 失敗。
            passed = res.status_code == 200 and ("similarCases" in data or data.get("riskScore", 0) >= 40)
            detail = f"狀態碼: {res.status_code}, Score: {data.get('riskScore')}, similarCases: {bool(data.get('similarCases'))}"
            result.add(case["name"], passed, detail)
        except Exception as e:
            result.add(case["name"], False, f"錯誤：{str(e)}")
        time.sleep(0.3)
    ...
    try:
        payload = {
            ...
            "summary_only": True,
            "screenshot_base64": "",
            "screenshot_skipped_reason": "unit_test_no_screenshot",
            "reported_reason": "測試摘要模式證據保全",
        }
        res = session.post(f"{BASE_URL}/api/submit_evidence", json=payload, timeout=TIMEOUT_SEC)
        data = safe_json_response(res)
        # Firebase 未連線或權限未授權時可以失敗；這筆重點是確認端點不再因缺圖直接 400。
        passed = res.status_code != 400 or "未提供圖片數據" not in str(data)
        result.add("證據保全摘要模式不因缺圖直接失敗", passed, f"狀態碼: {res.status_code}, 回應: {str(data)[:120]}")
    except Exception as e:
        result.add("證據保全摘要模式不因缺圖直接失敗", False, f"錯誤：{str(e)}")
...
if __name__ == "__main__":
    ...
    test_zero_budget_optimizations()
