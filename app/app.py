...
# ⚠️ Render 使用 gevent-websocket worker 時，必須在所有 import 前先做 gevent patch。
# 但完整 monkey.patch_all() 可能與 Python threading / Gunicorn worker 產生 KeyError，
# 因此這裡採用 thread=False：保留網路 I/O patch，同時避免修改 threading。
try:
